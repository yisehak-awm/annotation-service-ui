...
export class AnnotationService extends React.Component {
    ...
    handleAnnotationsChanged(isSelected, annotation) {
        this.setState(state => {
            ...
            if (isSelected) {
                ...
                biogrid = selectedAnnotations.find(a => a.name === "biogrid-interaction-annotation");
                if (annotation === "biogrid-interaction-annotation") {
                    ...
                    if (annotation === "gene-pathway-annotation") {
                        if (biogrid) { //edit biogrid
                            let index = selectedAnnotations.indexOf(biogrid);
                            selectedAnnotations[index] = {
                                name: "biogrid-interaction-annotation",
                                filter: {
                                    "interaction": "proteins"
                                }
                            }
    ...
    handleFilterChanged(annotation, filter) {
        this.setState(state => {
            let selectedAnnotations = state.selectedAnnotations.map(sa => {
                if (sa.name === annotation) {
                    sa.filter = Object.assign({}, sa.filter, filter);
                }
                return sa;
            });
            let biogrid = selectedAnnotations.find(a => a.name === "biogrid-interaction-annotation");
            if(annotation === "gene-pathway-annotation" && biogrid && filter.include_prot !== undefined){
                let index = selectedAnnotations.indexOf(biogrid);
                if(filter.include_prot){
                    selectedAnnotations[index] = {
                        name: "biogrid-interaction-annotation",
                         filter: {
                                "interaction": "Proteins"
